...
- [x] AI 全部失败后的一次性固定 48,458 C-grid 显式回退；原参数/生成器不变且不放宽门禁
...
- [x] 源码树完整测试通过（196 项，另有 7 个子测试）
- [x] `v2.0.2` 便携发布副本完成 Skill、引擎清单和完整测试
...
- [x] 固定 48,458 正常自动重试、63,040 自动候选和单指标自动择优均已移除；
      原 48,458 方法仅保留为 AI 全部失败后的显式受审回退
